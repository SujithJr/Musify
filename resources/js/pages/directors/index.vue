<template>
    <div>
        <div class="flex items-center justify-between">
            <h2 class="text-grey">Directors</h2>
            <template v-if="currentUser">
                <button type="button" v-if="currentUser.role_id === 1" class="btn btn-primary flex" data-toggle="modal" data-target="#exampleModalCenter">Create Director<i class="material-icons inline-block ml-2" style="vertical-align: middle;">local_movies</i></button>
            </template>
        </div>
        ...
        <div class="row mt-16">
            <div class="col-md-4 c-column mb-12" v-for="item in directors" :key="item.index">
...
export default {
    ...
    created () {
        this.$store.dispatch('directors/fetchDirectors')
    ...
    methods: {
        ...
        addDirector () {
            this.$store.dispatch('directors/createDirector', this.content)
            setTimeout(() => {
                this.removeImage()
            }, 100)
        },
